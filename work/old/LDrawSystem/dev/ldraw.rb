...
module LDraw
  ...
  @skpdir = File.join(Sketchup.find_support_file("Plugins"), "LDrawSystem", "parts")
...
def LDraw.add_part_in_model(pn)
...
def LDraw.start_operation(name)
  Sketchup.active_model.start_operation(name, true)
end
def LDraw.commit_operation
...
def LDraw.get_peeron_inventory
  ret = UI.inputbox(["Set Num"])
  listdir = File.join(Sketchup.find_support_file("Plugins"), "LDrawSystem", "partlists")
  $num = ret[0]
  $str = []
...
def LDraw.parse_inventory(page)
  #add_materials
  read_colors unless @colors
...
unless file_loaded?(scriptname)
  ...
  menu.add_item("Export to .ldr") { LDraw.export_model }

  menu.add_item("Import .ldr file") { LDraw.import }
  ...
  if mymenu
    ...
    menu.add_item("Random Colors") { LDraw.random_colors }
    menu.add_item("Peeron Inventory") { LDraw.get_peeron_inventory }
