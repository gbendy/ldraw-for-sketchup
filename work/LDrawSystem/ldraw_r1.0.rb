...
module LDrawSystem
  ...
  @model_dir = File.join("d:", "sketchup", "models")
  ...
  @skpdir = File.join(@model_dir, "LDraw")
...
def LDrawSystem.set_color(ent, code)
  LDrawSystem.read_colors unless @colors
  ary = @colors[code]
  return unless ary
...
def LDrawSystem.process_file(file, cdef, color)
  ...
  lines.each do |line|
	...
	if file
      ...
      #tr = Geom::Transformation.new(t.origin, X_AXIS, 90.degrees)
      #t *= tr if is_skp
...
def LDrawSystem.parse_inventory(page)
  ...
  parts.each_with_index do |part, i|
    q, n, color = part
    if( cdef = Sketchup.active_model.definitions[n] )
      # Keep going
    elsif ( File.exist?(file=File.join(@skpdir, n+".skp")) )
      cdef = Sketchup.active_model.definitions.load(file)
    else
      puts "#{n} not in #{@skpdir}"
      next
    end
    w = cdef.bounds.width
    h = cdef.bounds.height
    if pt.x > 700
      pt.x = 0
      pt.y += h + 160
    end
    ...
    (q.to_i).times do 
      pt.z += cdef.bounds.depth + 10
      tr1 = Geom::Transformation.new(pt)
      #tr2 = mod_trans(tr1, 0)
      ins = Sketchup.active_model.entities.add_instance(cdef,tr1)
      mat = Sketchup.active_model.materials[ color ]
      unless mat
	lcolor = @lcolors[color.downcase]
	if lcolor.nil?
	  puts "need color: #{color}"
	else
...
def LDrawSystem.wear_hat
  sels = Sketchup.active_model.selection.to_a
  if sels[0].definition.name[/3626/]
    head = sels[0]
    hat = sels[1]
  else
    head = sels[1]
  ...
  hat.transformation = head.transformation
end
def LDrawSystem.grow_hair()
  sels = Sketchup.active_model.selection.to_a
  if sels[0].definition.name[/3626/]
    head = sels[0]
    hair = sels[1]
  ...
  hair.transformation = head.transformation
end
...
unless file_loaded?(scriptname)
  ...
  if mymenu
    ...
    menu.add_item("Grow Hair") { LDrawSystem.grow_hair }
    menu.add_item("Wear Hat") { LDrawSystem.wear_hat }
